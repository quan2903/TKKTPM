...
<section className="p-5 bg-white rounded-2xl border border-solid border-slate-100 w-full">
  <div className="grid py-2 text-base font-semibold border-b border-slate-100 grid-cols-[2fr_1fr_1fr_2fr_2fr] text-slate-400">
    <div>Tên sân</div>
    <div>Ngày đặt</div>
    <div>Tổng giá</div>
    <div>Trạng thái</div>
    <div className="text-center">Hành động</div> {/* canh giữa header nút */}
  </div>
  ...
  {rows.length > 0 ? (
    rows.map((row, index) => {
      ...
      return (
        <div
          key={index}
          className="grid py-4 text-sm border-b border-dashed border-slate-200 grid-cols-[2fr_1fr_1fr_2fr_2fr] items-center"
        >
          <div>{row.name}</div>
          <div>{row.date}</div>
          ...
          <div className="flex justify-center gap-4"> {/* canh giữa và cách đều */}
